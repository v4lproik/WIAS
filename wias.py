...
def checkMod(value):
    try:
        ...
        raise argparse.ArgumentTypeError("Modules must be : " + str(conf.all_module))
...
def process(tab):
    ...
    flag_response, response = objReq.request(url=urlparse.urljoin(objReq.getUrl(),action), method=method, data=data)
    if flag_response:
        ...
        if htmlAnalyser.getDifferenceBetweenTwoPages(content,bad_cred) != 0:
            ...
            if not objHtml.amILoggedInGrep(diffs, login):
                objConf.printMessage("   [] Bad Credentials : " + login + "/" + pass_u, "v")
                return False, login, pass_u
            else:
                objConf.printMessage("   [] Potentionaly Good Credentials : " + login + "/" + pass_u, "v", "find")
                return True, login, pass_u
        else:
            objConf.printMessage("   [] Good Credentials : " + login + "/" + pass_u, "v", "find")
            return True, login, pass_u
...
def process_401(tab):
    ...
    flag_response, response = objReq.request(url=urlparse.urljoin(objReq.getUrl(),action))
...
if __name__ == "__main__":
    ...
    try:
        ...
        gr1.add_argument('-f', '--filename', dest='filename', required=True, help='File with IP or DOMAIN - One per line')
        gr1.add_argument('-db', '--database', dest='database', required=True, help='File with MD5:WEBName - One par line')
        gr1.add_argument('-m', '--module', dest='module', required=True, nargs='+', type=checkMod, help='List of module that be running. For more details regarding modules, try : -m list. Modules List : ' + str(conf.all_module))

        gr2 = parser.add_argument_group("optional arguments")
        gr2.add_argument('-v', '--verbose', dest='verbose', default=False,  action='store_true', help='Verbosity level')
        gr2.add_argument('-c', '--color', dest='color', default=False,  action='store_true', help='Display Color to stdin')
        gr2.add_argument('-a', '--aggressive', dest='aggressive', default=False,  action='store_true', help='Try to identify a login form through different process.')
        gr2.add_argument('-conf', '--conf_folder', dest='conf_folder', default="conf/",  action='store_true', help='Path of the conf folder where all the configuration files are stored')
        ...
        for domain in dic_domain:
            ...
            #Url Request
            objReq = webRequest(domain, UA=objConf.user_agent, proxy=objConf.proxy, timeout=objConf.http_timeout, cookie_redirection=objConf.redirect_cookie)
            ...
            if flag_response:
                ...
                if domain in MyHTTPRedirectHandler.permanent_redirection:
                    ...
                    objConf.printMessage(" [] 301 Found : " + domain + " -> " + objReq.domain, "r", "find")


                if response.code == 401:
                    objConf.printMessage(" [] The server returned : " +  str(response.code), "r")

                    #only bf is available for 401
                    if not flag_bf:
                        objConf.printMessage(" [] Only BruteForce Tests are available for Unauthorised response", "r", "error")
                        continue
                    flag_favicon = False
                    flag_hash = False
                    flag_form = False
                ...
                objConf.printMessage(" [] Error : " + str(response), "r", "error")
                continue  
            ...
            if flag_favicon:
                ...
                fav_response, fav = objReq.getFavicon(html_response=html_response)
                
                #favicon method
                md5_favicon = cryptoComputation.md5ChecksumContent(fav.read())
                if(fav_response):
                    objConf.printMessage("  [] Favicon md5 computation : " + md5_favicon, "r")
                    fav_db = objFav.isFaviconInDB(md5_favicon)
                    if fav_db:
                        objConf.printMessage("  [] Favicon belongs to : " + fav_db, "r")

                        #check if default pass stored
                        flag_favicon_found = objConf.isFaviconPass(md5_favicon)
                        if flag_favicon_found:
                             objConf.printMessage("  [] Default Passwords Found", "r", "find")
                    else:
                        objConf.printMessage("  [] No Favicon found in the Database", "r")
                        if objConf.entry_favicon:
                            html_title = htmlAnalyser.getTitle(html_response)
                            conf.setOneLineFileContent(favdb, md5_favicon + ":" + html_title + "\n")
                            objConf.printMessage("  [] Favicon added to the Database", "r")
                else:
                    objConf.printMessage("  [] No Favicon found !", "r", "error")
            else:
            ...
            #Form Tests : Extract All form and then try to identify a login form, according to the configuration file stored in "conf/"
            if flag_form:
                ...
                if flag_response:
                    ...
                    if(number > 0):
                        objConf.printMessage("  [] " + str(number) + " Form(s) found.", "r")

                        all_form = htmlAnalyser.getAllForm(content)
                        for form_u in all_form:
                            input_match, form = objHtml.isThereALoginForm(form_u, aggressive)
                        #print form
                        if input_match != []:
                            objConf.printMessage("  [] " + str(input_match), "v")
                        else:
                            objConf.printMessage("  [] Extracting Form Issue", "r", "error")


                        objConf.printMessage(" [*] Login Form Tests", "r", "info")
                        if len(input_match) > 0:
                            ...
                            method, action, data, res_u, res_p = objHtml.extractFormInput(form, objConf.pattern_login, objConf.pattern_pass)
                            
                            if res_p:
                                objConf.printMessage("  [] Login Input " + str(res_p), "v")

                            if res_u:
                                objConf.printMessage("  [] Password Input " + str(res_u), "v")

                            objConf.printMessage("  [] Method : " + method, "v")
                            objConf.printMessage("  [] Action : " + action, "v")
                            objConf.printMessage("  [] Data : " + str(data), "v")
                        else:
                            objConf.printMessage("  [] No Login Form Found", "r", "error")
                    else:
                       objConf.printMessage("  []  No Form Found", "r", "error")
            else:
            ...
            if flag_crawler and not flag_login_form:
                ...
                if crawler.login_form_link:
                    objConf.printMessage("  [] Login Form Found at : " + str(crawler.login_form_link), "r", "find")
                    flag_login_form = True
                    #extract information and try to submit
                    method, action, data, res_u, res_p = objHtml.extractFormInput(crawler.login_form, objConf.pattern_login, objConf.pattern_pass)
                    if res_p:
                        objConf.printMessage("  [] Login Input " + str(res_p), "v")
            ...
            if flag_login_form:
                ...
                if inc > 0:
                    ...
                    while inc_loop < inc and not flag_bf_found:
                        ...
                        if inc_loop == 0 and switch:
                            objConf.printMessage("  [] Test with Default Logins Passwords", "r")
                            if flag_favicon_found:
                                arr_default_login_password = objConf.getDefaultLoginPasswordByFavicon(md5_favicon)
                            elif flag_hash_found:
                                arr_default_login_password = objConf.getDefaultLoginPasswordByHash(md5_favicon)
                            else:
                                objConf.printMessage("  [] Error... Something Wrong happened during detection...", "r", "error")
                                continue
                        else:
                            objConf.printMessage("  [] Test with Dictionaries", "r")
                            switch = False

                        if flag_401:
                            pool = multiprocessing.Pool(nb_proc)
                            results = pool.imap(func=process_401, iterable=worker([switch, action]))
                            ...
                            if flag_bf_found:
                                objConf.printMessage("  [] Credentials Found : " + login + " " + password, "r", "find")
                            else:
                                objConf.printMessage("  [] No Credentials Found", "r", "error")
                        else:
                            ...
                            if flag_response:
                                ...
                                #print bad_cred
                                #print htmlAnalyser.getDifferenceBetweenTwoPages(testa, content)
                            else:
                                pass
                            ...
                            results = pool.imap(func=process, iterable=worker([switch, action, pattern_pass, pattern_login, bad_cred]))
                            for i, login, password in results:
                                if i:
                                    flag_bf_found = True
                                    break
                ...
                objConf.printMessage("  [] Skipped because no Login Form Found", "r", "error")
